...
date: 2011-11-01
description: "Replacing FreeNAS with Ubuntu Server using ZFS for storage pools and Samba for file sharing."
---
...
right?). As rj in the comments pointed out, there is now a PPA with ZFS on Ubuntu, so we'll use that
...
If you aren't using the PPA (installing from source), you'll need your own init script, so ZFS pools are mounted at startup.
...
without losing data. For example RAIDZ can lose one drive, RAIDZ2 can
...
Change `# security = user` to `security = user`, then add your shares:
